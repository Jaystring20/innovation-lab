...
import { createTeam, listMyTeams, listStages, listTeamSubmissions, type Stage, type Team, type Submission } from '@/lib/lab';
...
const Dashboard: React.FC = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setStages(s);
      setTeams(t);
      // Load submissions for the first team if available
      if (t[0]) {
        const subs = await listTeamSubmissions(t[0].id);
        setSubmissions(subs);
        setTier(t[0].division as TierType);
      }
                ...
                {teams.map((team, idx) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      listTeamSubmissions(team.id).then(setSubmissions);
                    }}
