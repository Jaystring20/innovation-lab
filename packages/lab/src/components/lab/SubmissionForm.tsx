    ...
    <form onSubmit={handleSave} className="space-y-4">
      ...
      {(stage.deliverable_kind === 'video' || stage.deliverable_kind === 'prompt_log') && (
        <>
          <FileUpload
            ...
            hint="Upload an MP4 or provide a YouTube/Vimeo/Drive link"
            currentUrl={payload.video_url}
      ...
      {stage.deliverable_kind === 'prototype' && (
        <>
          <FileUpload
            submissionId={submission.id}
            purpose="video"
            label="Demo — Upload video/photos or paste URL"
            hint="Show your prototype working (MP4, images, etc.)"
            currentUrl={payload.video_url}
            onUpload={(url) => set('video_url', url)}
            disabled={locked}
          />
      ...
      <>
        <FileUpload
          submissionId={submission.id}
          purpose="doc"
          label={stage.deliverable_kind === 'prompt_log' ? 'Prompt log — Upload or paste URL' : 'Code / docs — Upload or paste URL'}
          hint={
            stage.deliverable_kind === 'prompt_log'
              ? 'PDF, Markdown, or link to your prompt log'
              : 'PDF, doc, or repo link. Optional.'
          }
          currentUrl={payload.doc_url}
          onUpload={(url) => set('doc_url', url)}
          disabled={locked}
        />
        {!payload.doc_url && (
          <Field
            label={stage.deliverable_kind === 'prompt_log' ? 'Or paste prompt log URL' : 'Or paste code/docs URL'}
            hint={
              stage.deliverable_kind === 'prompt_log'
